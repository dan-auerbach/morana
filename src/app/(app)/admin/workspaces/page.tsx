  ...
  return (
    <div>
      <div style={{ marginBottom: "24px" }}>
        ...
        <div style={{ color: "var(--gray)", fontSize: "13px" }}>$ admin --workspaces --manage</div>
      </div>
      ...
      <div style={{ marginBottom: "20px", display: "flex", gap: "12px", alignItems: "center" }}>
        ...
        <div style={{ fontSize: "12px", color: "var(--gray)" }}>{workspaces.length} workspace{workspaces.length !== 1 ? "s" : ""}</div>
      </div>
      ...
      {error && <div style={{ color: "var(--red)", fontSize: "13px", marginBottom: "16px" }}>[ERROR] {error}</div>}
      ...
      {loading ? (
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {workspaces.map((ws) => {
            ...
            return (
              <div key={ws.id} style={{ border: "1px solid var(--border)", padding: "12px 16px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    <span
                      onClick={() => { setExpandedId(isExpanded ? null : ws.id); if (!isExpanded) loadDetail(ws.id); }}
                      style={{ color: "var(--white)", fontWeight: 700, fontSize: "14px", cursor: "pointer" }}
                    >
                      <span style={{ color: isExpanded ? "#ff8800" : "#444", fontSize: "10px", marginRight: "4px" }}>{isExpanded ? "▼" : "▶"}</span>
                      {ws.name}
                    </span>
                    <span style={{ color: "var(--gray)", fontSize: "11px", marginLeft: "8px" }}>/{ws.slug}</span>
                    {!ws.isActive && <span style={{ color: "var(--red)", fontSize: "10px", marginLeft: "8px" }}>[INACTIVE]</span>}
                  </div>
                  <div style={{ display: "flex", gap: "12px", fontSize: "10px", color: "var(--gray)" }}>
                    <span>{ws._count.members} members</span>
                    <span>{ws._count.conversations} convos</span>
                    <span>{ws._count.recipes} recipes</span>
                    {ws.maxMonthlyCostCents && <span style={{ color: "var(--yellow)" }}>cap: ${(ws.maxMonthlyCostCents / 100).toFixed(0)}</span>}
                  </div>
                </div>
                ...
                {isExpanded && expandedData?.workspace.id === ws.id && (
                  <div style={{ marginTop: "12px", padding: "12px", backgroundColor: "rgba(13, 17, 23, 0.8)", borderLeft: "2px solid #ff8800" }}>
                    <div style={{ fontSize: "10px", color: "#ff8800", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "8px" }}>
                      ...
                      {ws.maxMonthlyCostCents && <span style={{ color: "var(--gray)" }}> / ${(ws.maxMonthlyCostCents / 100).toFixed(0)} cap</span>}
                    </div>

                    <div style={{ fontSize: "10px", color: "var(--green)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "8px" }}>MEMBERS</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: "4px", marginBottom: "12px" }}>
                      {expandedData.workspace.members?.map((m) => (
                        <div key={m.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "4px 8px", backgroundColor: "var(--bg)", border: "1px solid var(--border)" }}>
                          <span style={{ fontSize: "12px", color: "var(--white)" }}>{m.user.email}</span>
                          <div style={{ display: "flex", gap: "4px", alignItems: "center" }}>
                            <span style={{ fontSize: "9px", color: m.role === "admin" ? "#ff8800" : "var(--gray)", fontWeight: 700, textTransform: "uppercase" }}>{m.role}</span>
                            <button
                              onClick={() => handleToggleMemberRole(ws.id, m.user.id, m.role)}
                              style={{ padding: "2px 6px", background: "transparent", border: "1px solid var(--gray)", color: "var(--gray)", fontFamily: "inherit", fontSize: "9px", cursor: "pointer" }}
                            >
                              {m.role === "admin" ? "DEMOTE" : "PROMOTE"}
                            </button>
                            <button
                              onClick={() => handleRemoveMember(ws.id, m.user.id)}
                              style={{ padding: "2px 6px", background: "transparent", border: "1px solid var(--red)", color: "var(--red)", fontFamily: "inherit", fontSize: "9px", cursor: "pointer" }}
                            >
                              REMOVE
                            </button>
                          </div>
                    ...
                    <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
                      ...
                      <button
                        ...
                        style={{ padding: "6px 12px", background: "transparent", border: "1px solid var(--green)", color: "var(--green)", fontFamily: "inherit", fontSize: "10px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}
                      >
                        + ADD
                      </button>
                    </div>
