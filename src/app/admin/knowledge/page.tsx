    ...
    <div>
      ...
      <div style={{ marginBottom: "24px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
          ...
          <span style={{ color: "#00e5ff", fontSize: "18px", fontWeight: 700 }}>KNOWLEDGE BASE</span>
        </div>
        <div style={{ color: "#5a6a7a", fontSize: "13px" }}>$ admin --knowledge --manage</div>
      </div>
      ...
      {showForm && (
        <div style={{ padding: "16px", border: "1px solid #00ff88", backgroundColor: "rgba(0, 255, 136, 0.03)", marginBottom: "20px" }}>
          <div style={{ fontSize: "11px", fontWeight: 700, color: "#00ff88", textTransform: "uppercase", letterSpacing: "0.1em", marginBottom: "12px" }}>NEW KNOWLEDGE BASE</div>
          ...
          <button onClick={handleCreateKB} style={{ padding: "6px 16px", background: "transparent", border: "1px solid #00ff88", color: "#00ff88", fontFamily: "inherit", fontSize: "12px", fontWeight: 700, cursor: "pointer", textTransform: "uppercase" }}>
            [  CREATE  ]
          </button>
        </div>
      )}
      ...
      {/* KB list */}
      {loading ? (
        <div style={{ color: "#00ff88", fontSize: "13px" }}><span style={{ animation: "blink 1s step-end infinite" }}>_</span> Loading...</div>
      ) : (
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {kbs.map((kb) => {
            const isExpanded = expandedId === kb.id;
            return (
              <div key={kb.id} style={{ border: `1px solid ${isExpanded ? "rgba(0, 229, 255, 0.3)" : "#1e2a3a"}`, backgroundColor: isExpanded ? "rgba(0, 229, 255, 0.02)" : "transparent" }}>
                {/* KB header */}
                <div
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <span style={{ color: isExpanded ? "#00e5ff" : "#444", fontSize: "10px" }}>{isExpanded ? "▼" : "▶"}</span>
                    <div>
                      <div style={{ color: "#e0e0e0", fontSize: "13px", fontWeight: 600 }}>{kb.name}</div>
                      {kb.description && <div style={{ color: "#5a6a7a", fontSize: "11px", marginTop: "2px" }}>{kb.description}</div>}
                    </div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    <span style={{ color: "#5a6a7a", fontSize: "11px" }}>{kb._count.documents} doc{kb._count.documents !== 1 ? "s" : ""}</span>
                    <span style={{ color: kb.isActive ? "#00ff88" : "#ff4444", fontSize: "10px", fontWeight: 700 }}>{kb.isActive ? "ACTIVE" : "OFF"}</span>
                  </div>
                </div>
                  ...
                  <div style={{ padding: "0 16px 16px", borderTop: "1px solid rgba(30, 42, 58, 0.5)" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px", margin: "12px 0" }}>
                      <button
                        ...
                        style={{ padding: "4px 12px", background: "transparent", border: "1px solid #00e5ff", color: "#00e5ff", fontFamily: "inherit", fontSize: "10px", cursor: "pointer", fontWeight: 700 }}
                      >
                        {uploading === kb.id ? "UPLOADING..." : "+ UPLOAD DOC"}
                      </button>
                      <button
                        onClick={(e) => { e.stopPropagation(); handleDeleteKB(kb.id); }}
                        style={{ padding: "4px 12px", background: "transparent", border: "1px solid #ff4444", color: "#ff4444", fontFamily: "inherit", fontSize: "10px", cursor: "pointer", fontWeight: 700 }}
                      >
                        DELETE KB
                      </button>
                    </div>
                      ...
                      <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
                        {kb.documents.map((doc) => (
                          <div key={doc.id} style={{ display: "flex", alignItems: "center", gap: "12px", padding: "6px 8px", backgroundColor: "rgba(13, 17, 23, 0.5)", borderLeft: `2px solid ${statusColor(doc.status)}` }}>
                            ...
                            <span style={{ color: "#e0e0e0", fontSize: "11px", flex: 1 }}>{doc.fileName}</span>
                            <span style={{ color: "#5a6a7a", fontSize: "10px" }}>{doc.chunkCount} chunks</span>
                            <span style={{ color: "#5a6a7a", fontSize: "10px" }}>{(doc.sizeBytes / 1024).toFixed(0)} KB</span>
                            <button
                              onClick={(e) => { e.stopPropagation(); handleDeleteDoc(kb.id, doc.id); }}
                              style={{ background: "transparent", border: "none", color: "#444", cursor: "pointer", fontFamily: "inherit", fontSize: "12px", padding: "2px 4px" }}
                    ...
                    <div style={{ fontSize: "10px", color: "#5a6a7a", marginTop: "8px" }}>
